...
def get_audit_issues(engagement_id: str = "ENG-2024-001") -> str:
    ...
    # Store in cache for the executor to use (executor generates the A2UI block)
    generate_kanban_a2ui_tool(engagement)

    # Return ONLY the text summary — the executor will programmatically append
    # the Kanban board A2UI block. We never ask the LLM to output the large
    # <a2ui-json> block because it's ~16KB and gets truncated.
