            ...
            async for event in self.runner.run_async(
                ...
                if hasattr(event, "is_final_response") and event.is_final_response():
                    ...
                    if answer_text:
                        ...
                        # ── Self-healing ──────────────────────────────────────
                        # If the LLM forgot to include the <a2ui-json> block
                        # after calling get_audit_issues, append it from cache.
                        has_a2ui = any(
                            isinstance(p.root, DataPart)
                            and p.root.metadata.get("mimeType") == A2UI_MIME_TYPE
                            for p in final_parts
                        )

                        # Only self-heal for board-rendering queries (not action responses)
                        is_board_query = (
                            not has_a2ui
                            and _LAST_ISSUES_DATA
                            and ui_event_part is None  # don't re-render on action responses
                        )

                        if is_board_query:
                            logger.info(
                                "Self-healing: LLM response missing <a2ui-json> block. "
                                "Appending Kanban board from cache..."
                            )
                            try:
                                ...
                                logger.info("✓ Self-healed: Kanban board appended.")
