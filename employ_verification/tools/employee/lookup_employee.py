...
def lookup_employee(
    ...
    try:
        ...
        # Definitively bad forwarded token (GE authorization misconfig).
        # Return a sentinel the executor can detect and replace with a
        # generic, non-technical message for the end user — see
        # agents/_base/base_executor.py.
        logger.error(f"  - OBO auth error querying BigQuery: {e}")
        return json.dumps({"error_code": OBOAuthError.ERROR_CODE, "error": str(e)})
