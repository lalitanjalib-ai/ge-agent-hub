...
def get_bigquery_client() -> bigquery.Client:
    ...
    Raises:
        OBOAuthError: if the forwarded token is definitively unusable (e.g. a
            Google id_token in google_direct mode). This propagates
            deliberately — see agents/_base/exceptions.OBOAuthError — instead
            of being swallowed into a silent ADC fallback, since that would
            mean queries silently run as the service account instead of the
            user. Callers (the employee tools) catch this specifically and
            convert it into the OBO_AUTH_ERROR sentinel JSON response.
    """
    ...
    try:
        ...
        # Definitively bad token type — do not fall back to ADC, re-raise so
        # the caller (tool function) can surface a clear, generic error.
        raise
