...
def _first_access_token_from_authorizations(authorizations: Any) -> str | None:
    ...
    We prefer an explicit ``access_token`` key. If the dict instead only has
    an ``id_token`` key (no ``access_token``), that is exactly the
    misconfiguration that produces the "BigQuery needs an OAuth access
    token" failure downstream — log it loudly here, at the point of
    extraction, rather than only warning much later when credentials are
    actually built.
    """
    if isinstance(authorizations, dict):
        for value in authorizations.values():
            if isinstance(value, dict):
                ...
                if value.get("id_token") and not value.get("access_token"):
                    logger.error(
                        "OBO: GE authorization payload contains only 'id_token' "
                        "(no 'access_token') — the GE authorization resource's "
                        "OAuth scopes/consent must be fixed so an access_token "
                        "is issued and forwarded; a Google id_token cannot be "
                        "exchanged for an access_token client-side"
                    )
                    continue
...
def find_google_token_in_mapping(state: Mapping[str, Any] | None) -> str | None:
    ...
    Prefers an explicit ``access_token`` key over bare strings/id_token keys,
    since a dict carrying only ``id_token`` (no ``access_token``) is the
    known GE misconfiguration that breaks BigQuery OBO in google_direct mode.
    """
    if not isinstance(state, Mapping):
        return None
    for value in state.values():
        if isinstance(value, Mapping):
            ...
            if isinstance(access, str) and _looks_like_google_access_token(access):
                return access
            if value.get("id_token") and not access:
                logger.error(
                    "OBO: found a mapping with 'id_token' but no 'access_token' "
                    "while scanning for a forwarded Google token — GE authorization "
                    "resource is not forwarding an access_token; fix its OAuth "
                    "scopes/consent (see agents/_base/exceptions.OBOAuthError)"
                )
                continue
            for inner in value.values():
...
def _credentials_from_google_access_token(
    ...
    *,
    ...
    Raises:
        OBOAuthError: if ``access_token`` is definitively NOT a usable OAuth
            access token (an Entra JWT, or a Google id_token). Neither case
            can be fixed client-side — Google does not support exchanging an
            id_token for an access_token, and an Entra JWT means the GE
            authorization resource is misconfigured for a different IdP.
            Silently wrapping either of these in ``Credentials(token=...)``
            just defers the failure to a confusing BigQuery-level "missing
            refresh_token/client_id/client_secret" error later, so we fail
            fast here instead.
    """
    ...
    if _looks_like_entra_jwt(access_token):
        msg = (
            "OBO google_direct mode received an Entra JWT, not a Google OAuth "
            "access token — the GE authorization resource must use Google OAuth "
            "(accounts.google.com), not Entra. This is a GE authorization "
            "resource misconfiguration, not something fixable in this request."
        )
        logger.error(msg)
        raise OBOAuthError(msg)

    if _looks_like_google_jwt(access_token):
        msg = (
            "OBO google_direct mode received a Google id_token instead of an "
            "OAuth access_token. BigQuery requires an access_token; a Google "
            "id_token CANNOT be exchanged for one client-side (unlike Entra, "
            "Google has no STS token-exchange path between these token types). "
            "Fix: the GE authorization resource's OAuth scopes/consent must be "
            "reconfigured so Google's token endpoint issues (and GE forwards) "
            "an access_token — see scripts/setup_agent_auth.py and re-consent "
            "the affected user. Falling back to ADC is intentionally NOT done "
            "here because it would silently run queries as the service account "
            "instead of the user, defeating the purpose of OBO."
        )
        logger.error(msg)
        raise OBOAuthError(msg)
...
def get_user_gcp_credentials(
    ...
    if mode == OBO_MODE_GOOGLE_DIRECT:
        # NOTE: OBOAuthError is intentionally NOT caught here. It indicates a
        # definitively unusable token (id_token, or wrong-IdP JWT) — falling
        # back to ADC in that case would silently run queries as the service
        # account instead of the user, which defeats the purpose of OBO. Let
        # it propagate so callers (bq_client.py / tools) can decide how to
        # surface it, rather than us deciding here to hide it.
