"""
Employee Verification Agent V2 — ADK Agent built from YAML config.

The heavy lifting (prompt construction, tool loading, A2UI schema) happens
here, but all configurable values come from config/employee_verification_v2.yaml.
"""
...
def create_agent() -> Agent:
    """Create and return the Employee Verification Agent V2 from config."""
    ...
    agent = Agent(
        name=agent_cfg.get("name", "EmployeeVerificationAgentV2"),
        model=model,
