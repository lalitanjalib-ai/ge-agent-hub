"""
emp_verify_google_oauth_v3 — Agent Authorization Setup Script

Creates (or verifies) the Gemini Enterprise agent_authorization resource
needed for A2UI/A2A agent integration. This is normally handled
automatically by scripts/deploy.py on first deploy — run this manually
only when you need to force-recreate the authorization (e.g. to fix a
GE misconfiguration where the OAuth consent is issuing/forwarding an
id_token instead of the required access_token).
...
Examples:
  ...
  python scripts/setup_agent_auth.py --id my-auth # Use a custom authorization ID
    ...
    if not args.check and (not oauth_client_secret or oauth_client_secret.strip() == "CHANGE_ME"):
        ...
        print("  Get the client secret for the GCP OAuth 2.0 Web client (OAUTH_CLIENT_ID)")
        print("  from GCP Console → APIs & Services → Credentials.")
        sys.exit(1)
